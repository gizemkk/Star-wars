...
function starWars(endpoint){
    ...
    .then(data=>{
        ...
        console.log(data)
        data.forEach(element => {console.log(`Film episode_${element.episode_id} title is ${element.title}`)})
       ...
       .catch((err)=>{ console.log(err)
        ...
        function starWars(endpoint){
          ...
          Promise.all([starWarsCount('films'),starWarsCount('planets'),starWarsCount('species'),
          starWarsCount('people'),starWarsCount('starships'),starWarsCount('vehicles')])
          .then(([films,planets,species,people,starships,vehicles])=>{
              output.innerHTML =  `${films.length} films ,` + `${planets.length} planets, ` + `${species.length} species ,` +
              `${people.length} people,` + `${starships.length} starships, ` + `${vehicles.length} vehicles, ` 
          }) .catch((err) =>{
              console.log(err)
